...
架構：Next.js + Supabase + PostgreSQL
...
## Outcome
Shop 最終沒有以最初規劃的完整電商產品形式完成，而是在重新檢視產品定位後轉為公開封存。但它也是目前對我工程能力影響最大的專案之一。它讓我第一次真正面對一個已經不能靠快速拼湊繼續維持的 codebase，並開始透過 domain modeling、type system、data modeling 與 access control 重新整理系統。
    `
};